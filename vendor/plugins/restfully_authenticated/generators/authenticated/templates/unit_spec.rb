...
describe <%= class_name %> do
  ...
  it 'resets password' do
    ...
    <%= class_name %>.authenticate('quentin@example.com', 'new password').should == <%= table_name %>(:quentin)
  end

  it 'does not rehash password' do
    <%= table_name %>(:quentin).update_attributes(:email => 'quentin2@example.com')
    <%= class_name %>.authenticate('quentin2@example.com', 'test').should == <%= table_name %>(:quentin)
  end

  it 'authenticates <%= file_name %>' do
    <%= class_name %>.authenticate('quentin@example.com', 'test').should == <%= table_name %>(:quentin)
  end
  ...
  it 'does not authenticate suspended <%= file_name %>' do
    <%= table_name %>(:quentin).suspend!
    <%= class_name %>.authenticate('quentin@example.com', 'test').should_not == <%= table_name %>(:quentin)
  end
...
protected
  def create_<%= file_name %>(options = {})
    record = <%= class_name %>.new({ :email => 'quire@example.com', :password => 'quire', :password_confirmation => 'quire' }.merge(options))
